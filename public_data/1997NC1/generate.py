...
if __name__ == "__main__":
    ...
    # ===================== User parameters =====================
    OUTPUT_DIR = "public_data/1997NC1"
    TRUE_IMAGE_DIR = os.path.join(OUTPUT_DIR, "image")
    OVERVIEW_FILE = "compare.png"

    FRAME_COUNT = 13
    FALLBACK_IMAGE_SIZE = (64, 64)  # (range, azimuth), only used if true images are missing

    # Radar physical resolutions.
    RANGE_RESOLUTION = 75.0       # m
    DOPPLER_RESOLUTION = 0.08     # Hz

    # Image grid spacing. Set these manually; they do not need to equal resolution.
    RANGE_GRID_SPACING = RANGE_RESOLUTION/5     # m/pixel
    DOPPLER_GRID_SPACING = DOPPLER_RESOLUTION/5   # Hz/pixel

    # Radar and spin parameters.
    RADAR_FREQUENCY = 8.57e9       # Hz; change this if needed
    ROTATION_PERIOD = 62.0 * 3600  # s
    ELEVATION_DEG = 45.0           # deg
    AZIMUTH_START_DEG = -20.0        # deg; tune this value manually

    # Time span read from the labelled 1997 NC1 collage: 2026 June 25, 07:53-11:20 UT.
    OBS_START_UTC = "2026-06-25T07:53:00Z"
    OBS_END_UTC = "2026-06-25T11:20:00Z"

    # Ellipsoid semi-axes in meters. These are radii, not diameters.
    # Full diameters are 2*a, 2*b, 2*c.
    ELLIPSOID_SEMI_AXES = (500.0, 250.0, 250.0)
    ...
    frame_azimuth_step_deg = 360.0 * frame_interval_sec / ROTATION_PERIOD

    print("Generating 13-frame ellipsoid preview, true/ellipsoid comparison, and radar_meta.json")
    print(f"- Output directory          : {OUTPUT_DIR}")
    print(f"- Comparison layout         : 7 true / 7 ellipsoid / 6 true / 6 ellipsoid / 7 overlay / 6 overlay")
    ...
    for row_idx, row_tiles in enumerate(rows):
        for col_idx, tile in enumerate(row_tiles):
            overview.paste(tile, (col_idx * width, row_idx * height))
